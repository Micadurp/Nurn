...
namespace Nurn
{
	...
	{
		sockaddr_in connectionaddr = connectionAddress.GetAddress();
...
#if PLATFORM == PLATFORM_MAC || PLATFORM == PLATFORM_UNIX
		int nonBlocking = 1;
		if (fcntl(OutSocketHandle, F_SETFL, O_NONBLOCK, nonBlocking) == -1)
#elif PLATFORM == PLATFORM_WINDOWS
		DWORD nonBlocking = 1;
		if (ioctlsocket(OutSocketHandle, FIONBIO, &nonBlocking) != 0)
#endif
		{
